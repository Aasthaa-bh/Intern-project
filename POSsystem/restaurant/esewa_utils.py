...
def generate_transaction_uuid():
    ...
    eSewa allows alphanumeric and hyphen.
    """
...
def decode_esewa_callback_data(encoded_data: str):
    """
    Decode Base64 callback data from eSewa success/failure redirect.
    """
...
def verify_esewa_payment(transaction_uuid, total_amount, product_code=None, timeout=15):
    ...
    try:
        ...
        return {
            "success": False,
            "message": f"Status check request failed: {str(e)}",
            "data": {},
        }
    except ValueError:
        return {
    ...
    return {
        "success": False,
        "message": f"Payment not complete. eSewa status: {status or 'UNKNOWN'}",
        "data": data,
    }